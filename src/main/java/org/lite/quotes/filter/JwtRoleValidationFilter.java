...
public class JwtRoleValidationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        if (authentication != null && authentication.getPrincipal() instanceof Jwt jwt) {
            ...
            // It's up to you which line do you want to enable, the latter one is more
            // secure, as you don't force the call to use a jwt token
            // filterChain.doFilter(request, response); // No JWT token, continue request
            // processing, i.e. calling the GET from browser
            response.setStatus(HttpServletResponse.SC_FORBIDDEN); // Return 403 Forbidden if role check fails, you are
                                                                  // forcing to use the token
        }
    }
